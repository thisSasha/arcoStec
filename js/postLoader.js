function domToObject(element) {
    ...
    obj[tagName] = {
        ...
        attributes: {},
        children: { childTagName: {} },
    };
    if (element.children.length > 0) {
        Array.from(element.children).forEach(child => {
            const childTagName = child.tagName.toLowerCase();
            obj[tagName].children[childTagName] = {
                innerHtml: child.innerHTML,
                attributes: {}
            };
            Array.from(child.attributes).forEach(attr => {
                obj[tagName].children[childTagName].attributes[attr.name] = attr.value;
            });
        });
    };
    ...
    Array.from(element.attributes).forEach(attr => {
        obj[tagName].attributes[attr.name] = attr.value;
    });
...
loadXMLDoc("./xml/xml.xml", function (xml) {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        htmlPost.onclick = function () { setPost(el) };
        htmlPost.innerHTML = `
